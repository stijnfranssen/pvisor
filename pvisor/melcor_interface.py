        ...
        while True:
            ...
            ### Variable name block, get the variable names and prepare for reading data.
                ...
                (n_vars, 
                 ...
                 start_byte, 
                 end_byte, 
                 unpack_fmt) = _prepare_data(var_names)
...
def _prepare_data(var_names: List[str]) -> Tuple[int, int, np.ndarray, int, int, int, str]:
    """
    ...
    return (n_vars, data_expand_size, data, t_step, start_byte, end_byte, unpack_fmt)
...
def _get_var_names(restart_file: BinaryIO) -> List[str]:
    ...
    (n_records,) = unpack("i", line[4:])  # Number of records (i.e. total number of variables)
    ...
    keywords: List[str] = [line[ii*keyword_length:(ii+1)*keyword_length].decode().strip() for ii in range(n_keys)]
    ...
    if len(var_names) != n_records:
        raise IOError(f"Invalid Melcor Plotfile!\n"
...
def _check_string(restart_file: BinaryIO, path: Path, string: str) -> str:
    ...
    if flag != string:
        raise IOError(f"'{path}' is not a valid MELCOR file.\n"
...
def _check_melcor_version(restart_file: BinaryIO) -> str:
    ...
    if byte == b"\x04":
        ...
        raise IOError(f"Start byte (byte) indicates that melcor version is not supported.")
