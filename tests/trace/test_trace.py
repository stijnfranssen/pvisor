...
def test_read_TRACE_demux():

    filename = Path("test_input.dmx")

    df = read_file(current_dir / filename, code="TRACE")

    df_expected = pd.read_csv(
        current_dir / Path("test_input.csv"),
        dtype=np.float32,
        index_col="time",
    )
...
def test_trace_demux_interpolation():
...
def test_trace_demux_time_vector_axial():
...
def test_trace_demux_axial_profile():
...
if __name__ == "__main__":
    ...
    test_read_TRACE_demux()
    test_trace_demux_interpolation()
    test_trace_demux_time_vector_axial()
    test_trace_demux_axial_profile()
